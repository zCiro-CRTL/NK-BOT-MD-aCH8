...
//globals :v
global.timeString = (seconds) => {seconds=Number(seconds);var d=Math.floor(seconds/(3600*24));var h=Math.floor(seconds%(3600*24)/3600);var m=Math.floor(seconds%3600/60);var s=Math.floor(seconds%60);var dDisplay=d>0?d+(d==1?" Dia ":" Dias "):"";var hDisplay=h>0?h+(h==1?" Hora ":" Horas "):"";var mDisplay=m>0?m+(m==1?" Minuto ":" Minutos "):"";var sDisplay=s>0?s+(s==1?" Segundo ":" Segundos "):"";return dDisplay+hDisplay+mDisplay+sDisplay}

global.formatDate = (n, locale = 'es') => {let d=new Date(n)
return d.toLocaleDateString(locale,{weekday:'long',day:'numeric',month:'long',year:'numeric',hour:'numeric',minute:'numeric',second:'numeric'})}

global.reacMoji = (from, client, emoji, m) => {const reactionEmoji={react:{text:emoji,key:m.key}}
client.sendMessage(from,reactionEmoji)}

global.miniLoc = async (img) => {var image=await jimp.read(img);var res=await image.resize(300,150).getBufferAsync(jimp.MIME_JPEG)
return res}

global.fetchJson = async (url, options) => {try{options?options:{}
const res=await axios({method:'GET',url:url,headers:{'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.69 Safari/537.36'},...options})
return res.data}catch(err){return err}}

global.getBuffer = async(url,options) => {try{options?options:{}
const res=await axios({method:"get",url,headers:{'DNT':1,'Upgrade-Insecure-Request':1},...options,responseType:'arraybuffer'})
return res.data}catch(e){return e}}

global.isUrl = (url) => {return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/,'gi'))}

global.pickRandom = (list) => {
  return list[Math.floor(Math.random() * list.length)]
}

global.traducIr = async (txttr) => {let apirres=await fetchJson(`https://latam-api.vercel.app/api/traductor?apikey=${MyApiKey}&idioma1=${MultiNK.Lengua()}&texto1=${txttr}`)
return apirres.traducido};
