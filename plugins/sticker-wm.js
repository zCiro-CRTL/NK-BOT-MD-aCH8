...
let handler = async (m, { conn, args, text, command, groupMetadata }) => {
  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      if (/video/g.test(mime)) if ((q.msg || q).seconds > 8) return m.reply(`*[ ! ] Máxima duración de vídeo son 7 segundos!*`)
      ...
      let out
      try {
        stiker = await sticker(img, false, teks1, teks2)
        ...
        if (!stiker) {
          ...
          stiker = await sticker(false, out, teks1, teks2)
        }
      }
    } else if (args[0]) {
      if (isUrl(args[0])) stiker = await sticker(false, args[0], teks1, teks2)
      else return m.reply('[ ! ] Url inválido, prueba con otro ;3')
    }
  } catch (e) {
    console.error(e)
    if (!stiker) stiker = e
